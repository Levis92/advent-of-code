...
# Format data
def format(data):
    return [{"direction": move[0], "steps": int(move[1:])} for move in data.split(",")]


with open("data/03.txt") as f:
    first_wire = format(f.readline())
    second_wire = format(f.readline())
...
def calc_steps(coord):
    first_steps = first_coords.index(coord) + 1
    second_steps = second_coords.index(coord) + 1
    return first_steps + second_steps


print(min(calc_steps(coord) for coord in intersections))
