...
fn main() {
    ...
    for report in contents.lines() {
        ...
        let diff: Vec<i32> = levels.windows(2).map(|pair| pair[1] - pair[0]).collect();
        if is_safe(&diff) {
            safe_count += 1;
        } else {
            for index in 0..diff.len() {
                let mut dampened_diff = diff.clone();
                if index + 1 < diff.len() && index != 0 {
                    dampened_diff[index + 1] += dampened_diff[index];
                }
                dampened_diff.remove(index);
