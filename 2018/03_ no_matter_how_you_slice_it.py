...
def get_number(pattern, line):
    return int(re.search(pattern, line).group(0))

def square_generator():
    for line in lines:
        yield {
            "id": get_number("(?<=#)\d*", line),
            "left": get_number("(?<=@ )\d*", line),
            "top": get_number("(?<=,)\d*", line),
            "width": get_number("(?<=: )\d*", line),
            "height": get_number("(?<=x)\d*", line),
        }
